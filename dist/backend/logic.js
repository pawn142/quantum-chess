...
export function isMoveAlwaysLegal(declaredMove, quantumPos, winByCheckmate = defaultSettings.winByCheckmate, fixedObjectIndex, fixedUnitIndex) {
...
export function checkPlayValidity(play, quantumPos, settings = defaultSettings) {
    ...
    if ([...play.primaryMoves, ...play.defaultMoves].some(declaredMove => {
        const stackedUnit = findUnit([playedObject], generateStartMiddleEnd(declaredMove.move)[2]);
        if (!stackedUnit) {
            return false;
        }
        const stackedObjectIndex = quantumPos.objects.indexOf(findObject(quantumPos, stackedUnit.state));
        const stackedUnitIndex = findObject(quantumPos, stackedUnit.state).units.indexOf(stackedUnit);
...
export function getCheckingDependencies(checkedCoords, quantumPos, filledPos = objectsToFilledPosition(quantumPos), firstMove, playedObject, side = quantumPos.otherData.whoseTurn) {
    if (firstMove) {
...
export function generateDependencies(declaredMove, quantumPos, winByCheckmate = defaultSettings.winByCheckmate) {
    ...
    const filledPos = objectsToFilledPosition({
...
export function cleanEntanglements(units) {
    units.forEach(unit => unit.entangledTo = unit.entangledTo.filter(entanglesUnit => units.includes(entanglesUnit)));
    return units;
}
export function makeMeasurement(quantumPos, dependency, measurementType = defaultSettings.measurementType, excludedSide, excludedObject, makeCopy = false) {
    const newQuantumPos = makeCopy ? positionalClone(quantumPos) : quantumPos;
    const dependentUnit = findUnit(newQuantumPos.objects.filter(objectSet => objectSet.pieceType.side !== excludedSide && objectSet !== excludedObject), dependency);
    const dependentObject = newQuantumPos.objects.find(objectSet => objectSet.units.includes(dependentUnit));
    const measurementSet = [dependentUnit, ...dependentObject.units.filter(unit => unit.entangledTo.includes(dependentUnit))];
    ...
    playedObject.units.slice().forEach(unit => {
        ...
        for (const primaryMove of localPrimaries) {
            if (generateMoveResults(primaryMove, newQuantumPos, settings.winByCheckmate, settings.measurementType)[1]) {
                ...
                promoted ||= !!primaryMove.move.end?.promotion;
        ...
        if (localDefault && defaultBuildup.numerator > 0 && generateMoveResults(localDefault, newQuantumPos, settings.winByCheckmate, settings.measurementType)[1]) {
            ...
            promoted ||= !!localDefault.move.end?.promotion;
...
export function detectCheckmate(quantumPos, explosiveCheckmate = defaultSettings.explosiveCheckmate, whoseTurn = quantumPos.otherData.whoseTurn, notInCheck = generatePossiblePositions(quantumPos, undefined, undefined, !explosiveCheckmate).every(completedPos => !isInCheck(completedPos, whoseTurn))) {
    if (notInCheck) {
        return false;
    }
...
export function areValidObjects(objectPosition) {
    try {
