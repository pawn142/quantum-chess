...
export function clearBoardElements() {
    for (let i = 0; i < 64; ++i) {
        const unitDiv = document.getElementById(i);
        if (unitDiv) {
            unitDiv.remove();
        }
    }
...
export function removeSelection() {
    for (let i = 0; i < 64; ++i) {
        const squareDiv = window["square" + i];
...
export function createArrow(start, end, type, checkPosition = false) {
    ...
    const hypotenuse = Math.sqrt(diff_x ** 2 + diff_y ** 2);
...
export function showPosition(objectPosition) {
    clearBoardElements();
    const mainPerspective = window.visualSettings.perspective === piece.Sides.white;
    ...
    for (const object of objectPosition.objects) {
        for (const unit of object.units) {
            const unitDiv = tools.createCover(window["square" + piece.coordToIndex(unit.state)]);
            ...
            if (window.visualSettings.showFullRings || unit.state.probability.lessThan(new Fraction)) {
                const probabilityDiv = tools.createCover(unitDiv);
            ...
            const unitImg = tools.createCover(unitDiv, "img");
...
export function setup() {
    ...
    for (const coord of piece.chessboard) {
        ...
        const squareButton = tools.createCover(squareDiv, "button");
        ...
        function handleClick(clickType) {
            ...
            if (!window.selection) {
                ...
                const matchingMove = logic.candidateMoves(piece.findUnit(piece.getSide(window.position), window.selection.coord), window.position).find(candidateMove => piece.areCoordsEqual(logic.generateStartMiddleEnd(candidateMove.move)[2], coord));
                const previousArrow = document.querySelector(`[id$="${window.selection.coord.x.toString() + window.selection.coord.y + coord.x + coord.y}"]`);
...
export function attemptChange(msgType = false) {
...
export function resetWinByMate(value = logic.isLegalPosition(window.position)) {
    if (value) {
        const original = window.gameSettings.winByCheckmate;
        window.gameSettings.winByCheckmate = false;
...
export function toggleInfiniteQubits(side) {
