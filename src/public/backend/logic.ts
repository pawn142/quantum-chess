...
export function getLeapCheckingPieces(completedPos: CompletedPosition, whoseTurn: keyof typeof Sides = completedPos.otherData?.whoseTurn ?? defaultData.whoseTurn, kingCoord: Coord | undefined = findPieceFromType(completedPos, Pieces.king, whoseTurn)?.state): CompletedSet[] {
	return kingCoord ? completedPos.pieces.filter(completedPiece => completedPiece.pieceType.side !== whoseTurn && isInRange({
...
export function generatePossiblePositions(quantumPos: ObjectPosition, fixedObjectIndex?: number, fixedUnitIndex?: number, endEarly: boolean = false): CompletedPosition[] {
...
export function isMoveAlwaysLegal(declaredMove: DeclaredMove, quantumPos: ObjectPosition, winByCheckmate: boolean = defaultSettings.winByCheckmate, fixedObjectIndex?: number, fixedUnitIndex?: number): boolean {
	return generatePossiblePositions(quantumPos, fixedObjectIndex, fixedUnitIndex).every(completedPos => isMoveLegal(declaredMove, completedPos, winByCheckmate));
}
...
export function checkPlayValidity(play: Play, quantumPos: ObjectPosition, settings: GameSettings = defaultSettings): Set<string> {
	...
	if ([...play.primaryMoves, ...play.defaultMoves].some(declaredMove => {
		const stackedUnit: PositionedPiece | undefined = findUnit([playedObject!], generateStartMiddleEnd(declaredMove.move)[2]);
		if (!stackedUnit) {
			return false;
		}
	...
	playedObject!.units.forEach((unit, unitIndex) => {
		...
		if (!settings.allowCastling && [...localPrimaries, ...localDefaults].some(declaredMove => moveType(declaredMove.move) === SpecialMoves.castle)) {
			problems.add("Castling is turned off in settings");
		}
		if (!settings.castleSplitting && localPrimaries.length > 1 && [...localPrimaries, ...localDefaults].some(declaredMove => moveType(declaredMove.move) === SpecialMoves.castle)) {
			problems.add("Making a split move while castling is turned off in settings");
		}
		if (localPrimaries.some(declaredMove => moveType(declaredMove.move) === SpecialMoves.castle && localPrimaries.some(otherMove => areCoordsEqual(generateStartMiddleEnd(declaredMove.move)[1][0]!, generateStartMiddleEnd(otherMove.move)[2])))) {
			problems.add("Cannot join the king and rook when making a split move while castling");
		}
		if (!settings.pawnDoubleMoveSplitting && localPrimaries.length > 1 && [...localPrimaries, ...localDefaults].some(declaredMove => moveType(declaredMove.move) === SpecialMoves.pawnDoubleMove)) {
			problems.add("Making a split move while making a pawn double move is turned off in settings");
		}
...
export function getCheckingDependencies(checkedCoords: Coord[], quantumPos: ObjectPosition, filledPos: CompletedPosition = objectsToFilledPosition(quantumPos), firstMove?: Move, playedObject?: ObjectSet, side: keyof typeof Sides = quantumPos.otherData.whoseTurn, hiddenCheck: boolean = false): Coord[] {
	if (firstMove) {
...
export function generateDependencies(declaredMove: DeclaredMove, quantumPos: ObjectPosition, winByCheckmate: boolean = defaultSettings.winByCheckmate): Coord[] {
	...
	const filledPos: CompletedPosition = objectsToFilledPosition({
		objects: quantumPos.objects.filter(objectSet => objectSet !== playedObject),
	...
	if (winByCheckmate) {
		...
		if (moveType(declaredMove.move) === SpecialMoves.castle) {
			currentDependencies.push(...getCheckingDependencies([significantSquares[0], significantSquares[1][0]!], quantumPos, structuredClone(filledPos), undefined, playedObject, undefined, true));
		}
	}
	...
	currentDependencies.forEach(dependency => {
		if (filteredDependencies.every(filteredDependency => !areCoordsEqual(filteredDependency, dependency))) {
...
export function makeMeasurement(quantumPos: ObjectPosition, dependency: Coord, measurementType: boolean = defaultSettings.measurementType, excludedSide?: keyof typeof Sides, excludedObject?: ObjectSet, makeCopy: boolean = false): [ObjectPosition, boolean] {
	const newQuantumPos: ObjectPosition = makeCopy ? positionalClone(quantumPos) : quantumPos;
	const dependentUnit: PositionedPiece = findUnit(newQuantumPos.objects.filter(objectSet => objectSet.pieceType.side !== excludedSide && objectSet !== excludedObject), dependency)!;
...
export function generateMoveResults(declaredMove: DeclaredMove, quantumPos: ObjectPosition, winByCheckmate: boolean = defaultSettings.winByCheckmate, measurementType: boolean = defaultSettings.measurementType, makeCopy: boolean = false): [ObjectPosition, boolean] {
	...
	while ((possiblePositions => possiblePositions.some(completedPos => isMoveLegal(declaredMove, completedPos, winByCheckmate)) && possiblePositions.some(completedPos => !isMoveLegal(declaredMove, completedPos, winByCheckmate)))(generatePossiblePositions(newQuantumPos, newQuantumPos.objects.indexOf(playedObject), unitIndex))) {
		makeMeasurement(newQuantumPos, generateRandomDependency(declaredMove, newQuantumPos, winByCheckmate), measurementType, undefined, playedObject);
	}
...
export function updateCastling(quantumPos: ObjectPosition): void {
	const castleValues: [boolean, boolean, boolean, boolean] = getCastleValues(objectsToGamePosition(quantumPos));
...
export function generatePlayResults(play: Play, quantumPos: ObjectPosition, settings: GameSettings = defaultSettings, makeCopy: boolean = false): [ObjectPosition, keyof typeof Sounds, keyof typeof Sides | false] {
	...
	playedObject.units.slice().forEach(unit => {
		...
		for (const primaryMove of localPrimaries) {
			if (generateMoveResults(primaryMove, newQuantumPos, settings.winByCheckmate, settings.measurementType)[1]) {
				...
				splitMoved ||= localPrimaries.length > 1;
				castled ||= moveType(primaryMove.move) === SpecialMoves.castle;
				promoted ||= !!(primaryMove.move as StandardMove).end?.promotion;
				...
				newEnpassant ||= moveType(primaryMove.move) === SpecialMoves.pawnDoubleMove;
			} else {
		...
		if (localDefault && defaultBuildup.numerator > 0 && generateMoveResults(localDefault, newQuantumPos, settings.winByCheckmate, settings.measurementType)[1]) {
			successfullyMoved = true;
			castled ||= moveType(localDefault.move) === SpecialMoves.castle;
			promoted ||= !!(localDefault.move as StandardMove).end?.promotion;
			...
			newEnpassant ||= moveType(localDefault.move) === SpecialMoves.pawnDoubleMove;
		}
	...
	for (const captureDependency of captureDependencies) {
		if (findUnit(getSide(newQuantumPos), captureDependency[0]) && findUnit(getSide(newQuantumPos, otherSide(quantumPos.otherData.whoseTurn)), captureDependency[1]) && (!measurePartiallyCaptured(findObject(originalPos, captureDependency[1])!.pieceType.type_p) || makeMeasurement(newQuantumPos, captureDependency[1], settings.measurementType, quantumPos.otherData.whoseTurn)[1]) && makeMeasurement(newQuantumPos, captureDependency[0], settings.measurementType, otherSide(quantumPos.otherData.whoseTurn))[1]) {
			...
			const capturedUnits: PositionedPiece[] = newQuantumPos.objects.find(objectSet => objectSet.units.includes(capturedUnit))!.units;
	...
	if (settings.winByCheckmate && !settings.explosiveCheckmate) {
		const kingUnits: PositionedPiece[] = findObjectFromType(newQuantumPos)!.units;
		kingUnits.slice().forEach(unit => {
			while (kingUnits.includes(unit) && getCheckingDependencies([unit.state], newQuantumPos).length) {
				makeMeasurement(newQuantumPos, chooseElement(getCheckingDependencies([unit.state], newQuantumPos)));
			}
		});
	}
...
export function candidateMoves(unit: PositionedPiece, quantumPos: ObjectPosition, side: keyof typeof Sides = quantumPos.objects.find(objectSet => objectSet.units.includes(unit))!.pieceType.side): DeclaredMove[] {
	...
	const currentMoves: Move[] = chessboard.map(coord => ({
...
export function areValidObjects(objectPosition: ObjectPosition): boolean {
	try {
...
export function isValidObjectsString(candidateString: string) {
	try {
		return areValidObjects(getObjectsFromString(candidateString)) && getObjectsString(getObjectsFromString(candidateString)) === candidateString;
	} catch {
		return false;
	}
}

export function checkEnpassant(quantumPos: ObjectPosition): boolean {
	return !quantumPos.otherData.enpassant || isCoord(quantumPos.otherData.enpassant) && [3, 6].includes(quantumPos.otherData.enpassant.y) && quantumPos.objects.some(objectSet => objectSet.pieceType.type_p === Pieces.pawn && objectSet.pieceType.side !== quantumPos.otherData.whoseTurn && objectSet.units.some(unit => !unit.state.promotion && areCoordsEqual(translateCoord(unit.state, 0, enpassantDisplacement(quantumPos.otherData.whoseTurn), true), quantumPos.otherData.enpassant as Coord))) && !areOfDifferentObjects(quantumPos, quantumPos.otherData.enpassant, translateCoord(quantumPos.otherData.enpassant, 0, -enpassantDisplacement(quantumPos.otherData.whoseTurn), true));
}

export function isLegalPosition(quantumPos: ObjectPosition): boolean {
	return areValidObjects(quantumPos) &&
	       ...
	       (!quantumPos.otherData.castling.canBlackCastleRight || getCastleValues(objectsToGamePosition(quantumPos))[3]);
}

export function isLegalPositionString(candidateString: string): boolean {
	return isValidObjectsString(candidateString) && isLegalPosition(getObjectsFromString(candidateString));
}

export function countPositions(objectPosition: ObjectPosition) {
...
export function totalProbability(objectSet: ObjectSet): Fraction {
