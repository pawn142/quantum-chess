...
export function isBlocked(move: Move, completedPos: CompletedPosition): boolean {
	...
	const blockedSquares: string[] = completedPos.pieces.map(completedPiece => JSON.stringify(completedPiece.position));
	return significantSquares[1].some(square => blockedSquares.includes(JSON.stringify(square)));
}

export function isEndpointBlocked(move: Move, completedPos: CompletedPosition): boolean {
	const significantSquares: [Coord, Coord[], Coord] = generateStartMiddleEnd(move);
	return getPiece(completedPos, significantSquares[0])!.pieceType.side === getPiece(completedPos, significantSquares[2])?.pieceType?.side;
}
