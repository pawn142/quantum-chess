...
export function makePlay(): void {
	...
	if (!logic.isPlayLegal(filteredPlay, window.position, window.gameSettings)) {
		...
		const playResults: [piece.ObjectPosition, keyof typeof tools.Sounds, keyof typeof piece.Sides | false] = logic.generatePlayResults(window.play, window.position, window.gameSettings);
